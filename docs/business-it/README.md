...
| Public DMZ | No Business IT access by default; documented exceptions only | Unsolicited access to user, server, printer, voice, guest, and management networks |
...
- The current NTP configuration is not synchronizing device clocks despite IP reachability to the server; 1 March 1993 is the IOS default epoch, not a valid operational timestamp.
