...
| --- | --- | --- |
| Hearthline application release | `0.2.0` | `VERSION` |
| Rust workspace packages | `0.2.0` | `Cargo.toml` workspace package |
| Svelte package | `0.2.0` | `web/package.json` |
| Appliance YAML schema | `0.3.0` | `APPLIANCE_SCHEMA_VERSION` |
| Connection YAML schema | `0.2.0` | `CONNECTION_SCHEMA_VERSION` |
| Generated appliance catalog schema | `0.3.0` | `FRONTEND_CATALOG_SCHEMA_VERSION` |
| Bootstrap process view schema | `0.2.0` | `web/src/generated/process-view.json` |
...
1. Select the next SemVer value and update `VERSION`.
2. Synchronize the Rust workspace, Cargo lockfile, npm package, and npm
   lockfile.
3. Run `node scripts/check-version.mjs` and resolve every mismatch.
4. Move completed entries from `Unreleased` into a dated changelog section.
5. Run Rust formatting, tests, Clippy, rustdoc, configuration validation, and
