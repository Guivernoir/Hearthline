...
| Implementation evidence | Partial | Rust parses and cross-validates 185 appliance, 231 connection, and 30 scenario YAML files; independent customer public paths, Business IT PC-01 through PC-04 internal DNS and HTTPS, Business IT core recovery, converged and timed northbound-firewall recovery with sync-loss, state-loss, stale-session, and fenced-isolation variants, typed factory operations-data with Forming collection and DMZ replication, three WAF-prevented paths, one customer access-circuit restoration, and one composite factory conduit-outage/local-command case execute with forwarding, delivery, denial, or control results; six workstations expose repeated ICMP diagnostics, selected application behavior, isolated DNS caches, and compatible persistent baseline runtimes with tested ARP/PAT retention, while resilience scenarios remain fresh; 14 HMI/SCADA sessions invoke selected Rust control behavior, Forming executes one bounded Structured Text sequence against Rust plant dynamics, and its SCADA can inspect control source, bounded historian tiers, and the brokered analytics path, while durable historian persistence, complete topology evaluation, general IEC 61131-3 execution, broader plant state, vendor-protocol HA, and broader process scenarios remain unimplemented |
...
| Operator interface evidence | Partial | All ten process areas expose Rust-backed operator sessions with YAML-derived instruments, permissions, safety permissives, actuator state domains, alarm acknowledgement, and operator-interface-to-vPLC-to-remote-I/O-to-actuator command traces; Forming adds one shared cell-wide SCADA/module-HMI runtime with a validated bounded Structured Text sequence, explicit I/O binding, 20-millisecond task, live measurements, output state, five process disturbances, named alarms, source inspection, bounded one-second historian acquisition and DMZ replication, and authorized replica publication with route evidence. This is not evidence of durable historian persistence, industrial subscription protocols, general IEC 61131-3 conformance, deterministic field-network timing, functional-safety behavior, or production plant-process fidelity |
