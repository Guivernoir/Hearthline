...
| Implementation evidence | Partial | Rust parses and cross-validates 162 appliance, 208 connection, and 28 scenario YAML files; independent customer public paths, Business IT PC-01 through PC-04 internal DNS and HTTPS, Business IT core recovery, converged and timed northbound-firewall recovery with sync-loss, state-loss, stale-session, and fenced-isolation variants, factory operations-data, three WAF-prevented paths, one customer access-circuit restoration, and one composite factory conduit-outage/local-command case execute with forwarding, delivery, denial, or control results; six workstations and ten HMI sessions invoke selected Rust behavior, while complete topology evaluation, IEC 61131-3 execution, changing plant state, vendor-protocol HA, and broader process scenarios remain unimplemented |
...
| Operator interface evidence | Partial | All ten process areas expose Rust-backed HMI sessions with YAML-derived instruments, permissions, safety permissives, actuator state domains, alarm acknowledgement, and HMI-to-vPLC-to-remote-I/O-to-actuator command traces; this is not evidence of IEC 61131-3 execution, deterministic field-network timing, functional-safety behavior, or plant-process fidelity |
