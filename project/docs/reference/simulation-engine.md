...
**Scope:** Behavior contracts, fixed-capacity runtime, YAML validation adapters, local configuration API, frontend projection, and reusable network, connector, and OT primitives
...
| Runtime | Borrowed registry for up to 192 components, deterministic fixed-capacity queues, 256 links, delayed delivery, trace records, capacity failures, and event limits |
...
in adapter crates. The workspace currently contains 35 appliance kinds and 43
...
validated editing service. CI also executes bounded identifier and
appliance-YAML fuzz campaigns.
