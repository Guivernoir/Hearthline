...
use hearthline_engine::{ConfigRepository, ConnectionRepository, FrontendApplianceCatalog};
...
impl ProjectPaths {
    fn from_project_root(root: &Path) -> Self {
        Self {
            appliance_root: root.join("config/appliances"),
            connection_root: root.join("config/connections"),
            generated_catalog: root.join("web/src/generated/appliance-configs.json"),
        }
    }
