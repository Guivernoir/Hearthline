...
use hearthline_engine::{
    ConfigRepository, ConnectionRepository, Effect, LinkAppliance, LinkMode,
    RENDERED_ROLE_CONTRACTS, ServiceNode, Simulator, appliance_contracts,
};
...
fn validate_configs() -> Result<(), Box<dyn Error>> {
    let appliances = ConfigRepository::load("config/appliances")?;
    let connections = ConnectionRepository::load("config/connections", &appliances)?;
    println!(
...
fn generate_frontend_configs() -> Result<(), Box<dyn Error>> {
    let appliances = ConfigRepository::load("config/appliances")?;
    let connections = ConnectionRepository::load("config/connections", &appliances)?;
    let json = serde_json::to_string_pretty(&appliances.frontend_catalog(&connections))? + "\n";
    let output = Path::new("web/src/generated/appliance-configs.json");
