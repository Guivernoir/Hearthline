...
use hearthline_sim::InteractiveScenarioSession;
...
fn customer_session_retains_and_expires_arp_and_pat_state() {
    ...
    let inactive = run_workstation_action_with_session(
        &appliances,
        &connections,
        &scenarios,
        "customer-pc-01",
    ...
    .expect("inactive runtime inspection");
    assert!(
        inactive
            .output
            .iter()
            .any(|line| line.contains("No network state"))
    );
    ...
    for (appliance, command, expected) in [
        ("customer-rtr-01", "show status", "Simulation time"),
        ("customer-sw-01", "show mac address-table", "VLAN"),
        ("customer-rtr-01", "show arp", "192.168.0.2"),
        ("business-frw-01a", "show sessions", "Proto"),
    ] {
        ...
        .expect("runtime inspection command");
        assert!(matches!(report.status, WorkstationActionStatus::Completed));
        assert!(report.output.iter().any(|line| line.contains(expected)));
    }

    for command in ["show unsupported", &"x".repeat(129)] {
        let report = run_workstation_action_with_session(
            &appliances,
            &connections,
            &scenarios,
        ...
        .expect("rejected runtime inspection command");
        assert!(matches!(
            report.status,
            WorkstationActionStatus::Unsupported
        ));
    ...
    for (appliance, command) in [
        ("customer-sw-01", "show mac address-table"),
        ("customer-rtr-01", "show arp"),
        ("customer-rtr-01", "show ip nat translations"),
        ("business-frw-01a", "show sessions"),
    ] {
        let empty = run_workstation_action_with_session(
            &appliances,
            &connections,
            &scenarios,
            "customer-pc-01",
        ...
        .expect("empty runtime table");
        assert!(empty.output.iter().any(|line| line == "No active entries."));
    }
