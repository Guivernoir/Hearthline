...
use hearthline_config::{
    ...
    ScenarioRepository, ScenarioTransportConfig,
};
use hearthline_operator::{
    WorkstationAction, WorkstationActionStatus, run_workstation_action, workstation_profile,
};
...
#[test]
fn terminal_and_curl_parsers_reject_ambiguous_or_unbounded_input_forms() {
    let (appliances, connections, scenarios) = repositories();
    let mut commands = vec![
        "help trailing\\".to_owned(),
        format!(
            "help {}",
            (0..33)
        ...
        "curl -X".into(),
        "curl -d".into(),
        "curl --unknown https://shop.hearthline.test/shop".into(),
        "curl https://shop.hearthline.test/shop https://shop.hearthline.test/other".into(),
        "curl -I -d x https://shop.hearthline.test/shop".into(),
        "curl".into(),
        "curl -d one -d two https://shop.hearthline.test/shop".into(),
        format!(
            "curl -d {} https://shop.hearthline.test/shop",
            "x".repeat(257)
        ),
    ];
    commands.extend([
        "curl --request=PUT https://shop.hearthline.test/shop".into(),
        "curl -XPATCH https://shop.hearthline.test/shop".into(),
        "curl --data=value https://shop.hearthline.test/shop".into(),
        "curl --data-raw=value https://shop.hearthline.test/shop".into(),
        "curl -dvalue https://shop.hearthline.test/shop".into(),
        "curl -X OPTIONS https://shop.hearthline.test/shop".into(),
        "help 'quoted argument' escaped\\ argument".into(),
    ]);
    ...
    for command in commands {
        ...
        .expect("bounded terminal parser report");
