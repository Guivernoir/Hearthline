...
use hearthline_engine::{
    ConnectionMedium, CopperCategory, CopperMedium, CopperWiring, Effect, FAMILY_CONTRACTS,
    LearningSwitch, LinkAppliance, LinkEndpoint, LinkMode, MediaLink, MediaLinkConfig, PortDuplex,
    ...
    assert!(
        contracts
            ...
            .all(|contract| !contract.baseline.is_empty()
                && !contract.schema_mapping.is_empty()
                && contract.snapshot.version > 0
                && contract.snapshot.includes_authoritative_state
                && contract.runtime_object_bytes > 0)
    );
    let families = FAMILY_CONTRACTS
        .iter()
        .map(|contract| contract.family)
        .collect::<BTreeSet<_>>();
    assert_eq!(
        FAMILY_CONTRACTS.len(),
        hearthline_model::BehaviorFamily::ALL.len()
    );
    assert_eq!(families.len(), hearthline_model::BehaviorFamily::ALL.len());
}
