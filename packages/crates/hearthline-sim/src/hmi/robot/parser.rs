...
use hearthline_model::{Angle, FixedValue, Percentage, Position};
        ...
        if let Some(code) = motion_code {
            let instruction = match code {
                0 | 1 => {
                    ...
                    for (letter, value) in &words {
                        match *letter {
                            'X' => target.x = position_coordinate(current.x, *value, absolute)?,
                            'Y' => target.y = position_coordinate(current.y, *value, absolute)?,
                            'Z' => target.z = position_coordinate(current.z, *value, absolute)?,
                            'A' => target.w = angle_coordinate(current.w, *value, absolute)?,
                            'B' => target.p = angle_coordinate(current.p, *value, absolute)?,
                            'C' => target.r = angle_coordinate(current.r, *value, absolute)?,
                            _ => {}
                        }
                    }
                    let speed_percent = percentage(parameter(&words, 'F').unwrap_or(20.0))?;
                    current = target;
                ...
                28 => {
                    ...
                    RobotInstruction::Move {
                        ...
                        speed_percent: percentage(parameter(&words, 'F').unwrap_or(20.0))?,
                    }
                }
                _ => {
                    return Err(line_error(
...
fn position_coordinate(
    ...
    let requested = Position::from_raw(quantized(requested)?.raw() / 1_000);
    Ok(if absolute {
        requested
    } else {
...
fn angle_coordinate(current: Angle, requested: f64, absolute: bool) -> Result<Angle, ConfigError> {
    let requested = Angle::from_raw(quantized(requested)?.raw() / 1_000);
    Ok(if absolute {
        requested
    } else {
        current.saturating_add(requested)
...
fn percentage(value: f64) -> Result<Percentage, ConfigError> {
    Ok(Percentage::from_raw(quantized(value)?.raw() / 10_000))
}

fn quantized(value: f64) -> Result<FixedValue, ConfigError> {
    format!("{value:.6}")
        .parse()
        .map_err(|error| ConfigError::new(format!("cannot quantize robot program value: {error}")))
}
