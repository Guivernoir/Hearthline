use super::super::quantities::project;
use hearthline_engine::{
    CeramicSlipBatch, FormingFault, FormingMeasurements, FormingOutputs, FormingPhase,
    FormingPhysicsInputs, FormingProcess, FormingSetpoints, FormingTrip, SequenceInputs,
};
...
impl MouldProcessRuntime {
    ...
    pub(in crate::hmi) fn start(&mut self, safety_ready: bool) -> Result<(), &'static str> {
        ...
        if self.paused {
            let control = self.program.control_state();
            self.process
                .start_controlled(safety_ready, control.phase)
                .map_err(|_| "The paused mould could not resume.")?;
            self.process.apply_control_state(control);
            self.paused = false;
            return Ok(());
        }
    ...
    pub(super) fn tick(
        ...
        while remaining > 0 && !self.paused {
            ...
            let before_control = self.program.control_state();
            let physics = self.process.advance_controlled(FormingPhysicsInputs {
                elapsed_ms: slice,
            ...
            remaining -= slice;
            if let Some(trip) = physics.trip {
                self.program.force_fault();
                self.process
                    .apply_control_state(self.program.control_state());
                self.production_enabled = false;
                self.stop_request = None;
                result.trip = Some(trip);
                break;
            }
            self.program.scan_with_physics_feedback(
                slice,
                SequenceInputs {
            ...
            let control = self.program.control_state();
            self.process.apply_control_state(control);
            if control.phase != before_control.phase {
                ...
                if self.stop_request == Some(MouldStopRequest::AfterPhase) {
                    ...
                    if control.phase == FormingPhase::Idle {
                        ...
                        self.process.pause_controlled(control);
                        self.paused = true;
                    }
                    break;
                }
    ...
    pub(super) fn snapshot(&self) -> HmiMouldProcessState {
        HmiMouldProcessState {
            ...
            casting_pressure_bar: project(self.process.setpoints().pressure_bar),
    ...
    pub(in crate::hmi) fn apply_parameter(
        &mut self,
        parameter_id: &str,
        value: FixedValue,
    ) -> bool {
