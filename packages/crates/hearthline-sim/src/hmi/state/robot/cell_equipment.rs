...
use crate::hmi::state::RuntimeSignal;
use crate::hmi::{HmiActuator, HmiSafety};
use hearthline_model::{FixedValue, Percentage};

use super::super::PlantControlRuntime;
use super::quantities::percentage_to_f64;

const TRANSFER_TRAVEL_MS: u64 = 1_600;
...
impl HandoffStationRuntime {
    ...
    fn tick(&mut self, elapsed_ms: u64) {
        let delta = Percentage::from_raw(
            elapsed_ms
                .saturating_mul(10_000)
                .checked_div(TRANSFER_TRAVEL_MS)
                .unwrap_or_default()
                .try_into()
                .unwrap_or(i64::MAX),
        );
        match self.state {
            TransferState::MovingToOperator => {
                self.progress = self
                    ...
                    .clamp(Percentage::ZERO, Percentage::from_raw(10_000));
                if self.progress >= Percentage::from_raw(10_000) {
            ...
            TransferState::MovingToCell => {
                self.progress = self
                    ...
                    .clamp(Percentage::ZERO, Percentage::from_raw(10_000));
                if self.progress == Percentage::ZERO {
    ...
    fn snapshot(&self) -> HmiHandoffStationState {
        HmiHandoffStationState {
            ...
            progress_percent: percentage_to_f64(self.progress),
...
impl GuardedCellRuntime {
    ...
    pub(in crate::hmi) fn sync_io(
        &self,
        signals: &mut [RuntimeSignal],
        actuators: &mut [HmiActuator],
    ) {
        set_signal(
            signals,
            &self.gate_position_sensor,
            FixedValue::from_integer(i64::from(!self.gate_open)),
        );
        for station in &self.handoffs {
            set_signal(
                signals,
                &station.in_cell_sensor,
                FixedValue::from_integer(i64::from(station.state == TransferState::InCell)),
            );
            set_signal(
                signals,
                &station.operator_side_sensor,
                FixedValue::from_integer(i64::from(station.state == TransferState::OperatorSide)),
            );
...
fn set_signal(signals: &mut [RuntimeSignal], tag: &str, value: FixedValue) {
