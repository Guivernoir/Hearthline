...
use hearthline_model::Angle;

use super::quantities::{angle_to_degrees, position_to_mm};
...
impl RobotRuntime {
    ...
    fn validate_active_station_pose(&mut self, pickup: bool) -> Result<(), RobotAutomaticFault> {
        ...
        let tolerance = if pickup {
            handoff.pickup_tolerance
        } else {
            handoff.handoff_tolerance
        };
        ...
        let translation_error_squared = translation_error_squared(actual, expected);
        let tolerance_squared = i128::from(tolerance.raw()).pow(2);
        let orientation_error = orientation_error(actual, expected);
        if translation_error_squared <= tolerance_squared
            && orientation_error <= handoff.orientation_tolerance
        {
            return Ok(());
        }
        let translation_error_mm = (translation_error_squared as f64).sqrt() / 1_000.0;
        let orientation_error_deg = angle_to_degrees(orientation_error);
        ...
        Err(self.fault(
            ...
            format!(
                ...
                position_to_mm(tolerance),
                angle_to_degrees(handoff.orientation_tolerance)
            ),
        ))
    }
...
fn translation_error_squared(actual: RobotPose, expected: RobotPose) -> i128 {
    let x = i128::from(actual.x.raw() - expected.x.raw());
    let y = i128::from(actual.y.raw() - expected.y.raw());
    let z = i128::from(actual.z.raw() - expected.z.raw());
    x.saturating_mul(x)
        .saturating_add(y.saturating_mul(y))
        .saturating_add(z.saturating_mul(z))
}

fn orientation_error(actual: RobotPose, expected: RobotPose) -> Angle {
    let raw = actual
        .w
        .raw()
        .abs_diff(expected.w.raw())
        .max(actual.p.raw().abs_diff(expected.p.raw()))
        .max(actual.r.raw().abs_diff(expected.r.raw()));
    Angle::from_raw(i64::try_from(raw).unwrap_or(i64::MAX))
}
