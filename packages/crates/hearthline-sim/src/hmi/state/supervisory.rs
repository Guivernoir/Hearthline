...
use super::super::{
    HmiAlarm, HmiAuditEntry, HmiSupervisoryAsset, HmiSupervisoryEvent, HmiSupervisoryIdentity,
    HmiSupervisoryNode, HmiSupervisoryRepository, HmiSupervisorySample, HmiSupervisoryState,
    HmiSupervisoryTag, HmiSupervisoryTemplate,
};
...
pub(in crate::hmi) struct SupervisoryRuntime {
    ...
    samples: BTreeMap<String, Vec<RuntimeSupervisorySample>>,
}

#[derive(Clone, Copy, Debug)]
struct RuntimeSupervisorySample {
    timestamp_ms: u64,
    value: FixedValue,
    quality_good: bool,
}

impl RuntimeSupervisorySample {
    fn snapshot(self) -> HmiSupervisorySample {
        HmiSupervisorySample {
            timestamp_ms: self.timestamp_ms,
            value: super::super::quantities::project(self.value),
...
impl SupervisoryRuntime {
    ...
    pub(in crate::hmi) fn tick(&mut self, elapsed_ms: u64, signals: &[RuntimeSignal]) {
        ...
        for (tag, samples) in &mut self.samples {
            ...
            samples.push(RuntimeSupervisorySample {
    ...
    fn tags(&self, signals: &[RuntimeSignal]) -> Vec<HmiSupervisoryTag> {
        self.profile
            .history
            .tags
            .iter()
            .filter_map(|tag| {
                ...
                Some(HmiSupervisoryTag {
                    tag: tag.clone(),
                    value: super::super::quantities::project(signal.value),
                    ...
                    samples: self
                        ...
                        .flatten()
                        .copied()
                        .map(RuntimeSupervisorySample::snapshot)
                        .collect(),
                })
            })
            .collect()
