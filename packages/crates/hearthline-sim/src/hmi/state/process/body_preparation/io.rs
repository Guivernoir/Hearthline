...
pub(super) fn sync(session: &mut PlantControlRuntime) {
...
fn signal_values(m: BodyPreparationMeasurements) -> Vec<(&'static str, FixedValue)> {
    let route = |id| {
        m.water_networks
            .routes
            .iter()
    ...
    vec![
        ...
        ("area-01-rw-ph-01", active_return_quality.ph + fixed!(0.2)),
        (
            "area-01-rw-cnd-01",
            active_return_quality.conductivity_us_cm + fixed!(600.0),
        ),
        (
            "area-01-rw-tt-01",
            active_return_quality.temperature_c + fixed!(1.0),
        ),
...
fn route_loss_percent(route: hearthline_engine::WaterRouteMeasurements) -> FixedValue {
    if route.inlet_flow_l_min <= FixedValue::ZERO {
        fixed!(0.0)
    } else {
        ((route.inlet_flow_l_min - route.outlet_flow_l_min) / route.inlet_flow_l_min
            * fixed!(100.0))
