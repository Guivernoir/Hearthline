...
use crate::hmi::{HmiAlarmSeverity, HmiControlMode};
...
pub(in crate::hmi) use body_preparation::{
    setpoints_from_parameter_groups, setpoints_from_parameters,
};

impl PlantControlRuntime {
    pub(in crate::hmi) fn tick(&mut self, elapsed_ms: u64) {
        if self.tick_body_preparation(elapsed_ms) {
            return;
        }
        ...
        let filling = self
            ...
            .count();
        let consumed = FixedValue::from_integer(i64::try_from(filling).unwrap_or(i64::MAX))
            * fixed!(0.8)
            * FixedValue::from_u64_ratio(elapsed_ms, 1_500);
        self.shared_tank_level_percent = self
            .shared_tank_level_percent
            .saturating_sub(consumed)
            .clamp(FixedValue::ZERO, fixed!(100.0));
        ...
        {
            self.controller.program = Some(Box::new(program));
        }
    ...
    pub(in crate::hmi) fn reset_faulted_moulds(&mut self) -> Vec<String> {
        let targets = self.moulds.keys().cloned().collect::<Vec<_>>();
        targets
            .into_iter()
            .filter(|target| {
    ...
    fn sync_process_snapshot(&mut self) {
        ...
        for (station, measurements, outputs) in stations {
            ...
            let inclination = measurements.mould_position_mm / fixed!(600.0) * fixed!(90.0);
            for (tag, value) in [
    ...
    fn sync_shared_signals(&mut self) {
        let timestamp_ms = self
            ...
            .unwrap_or_default()
            .saturating_mul(FormingProcess::SCAN_INTERVAL_MS);
        let sum = |value: fn(&hearthline_engine::FormingMeasurements) -> FixedValue| {
            self.moulds
                .values()
                .map(|mould| value(mould.measurements()))
                .fold(FixedValue::ZERO, FixedValue::saturating_add)
        };
        let max = |value: fn(&hearthline_engine::FormingMeasurements) -> FixedValue| {
            self.moulds
                .values()
                .map(|mould| value(mould.measurements()))
                .max()
        ...
        let min = |value: fn(&hearthline_engine::FormingMeasurements) -> FixedValue| {
            self.moulds
                .values()
                .map(|mould| value(mould.measurements()))
                .min()
    ...
    fn sync_robot_signals(&mut self) {
        ...
        let piece_gripped = if snapshot.gripper_closed {
            FixedValue::ONE
        } else {
            FixedValue::ZERO
        };
    ...
    pub(super) fn set_signal(&mut self, tag: &str, value: FixedValue, timestamp_ms: u64) {
        if let Some(signal) = self.signals.iter_mut().find(|signal| signal.tag == tag) {
            update_signal(signal, value, timestamp_ms);
        }
    }

    pub(super) fn set_actuator(&mut self, tag: &str, state: &str) {
...
fn update_signal(signal: &mut RuntimeSignal, value: FixedValue, timestamp_ms: u64) {
