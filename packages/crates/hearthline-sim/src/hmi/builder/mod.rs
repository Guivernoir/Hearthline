...
use hearthline_model::{FixedValue, ProcessEvent, ProcessSignal, SignalValue, Text};

use crate::{BehaviorConfig, ConfigError, ConfigRepository};

use super::actions::process::load_control_program;
use super::state::{
    ControllerRuntime, HmiOperatorContext, PlantControlRuntime, RemoteIoRuntime, RuntimeSignal,
    SupervisoryRuntime, setpoints_from_parameters,
};
use super::{HmiActuator, HmiAlarm, HmiAlarmSeverity, HmiPermissive, HmiSafety};
...
impl PlantControlRuntime {
    pub fn from_repository(appliances: &ConfigRepository, id: &str) -> Result<Self, ConfigError> {
        ...
        let shared_tank_level_percent = if moulds.is_empty() {
            ...
            super::quantities::quantize(forming_initial_value(
                appliances,
                &loaded.config.environment,
                &loaded.config.zone,
                "area-02-lt-01",
        ...
        let signal_scope = signals.iter().map(|signal| signal.tag.clone()).collect();

        let mut session = Self {
            context: HmiOperatorContext {
                ...
                signal_scope,
                safety_scope,
                supervisory_visible: supervisory_profile.is_some(),
            },
...
fn controller_runtime(
    ...
    let program = load_control_program(appliances, id)?.map(Box::new);
...
fn sample_signal(
    ...
    maximum: f64,
    initial_value: f64,
) -> Result<RuntimeSignal, ConfigError> {
    ...
    sensor.set_raw_value(
        format!("{initial_value:.6}")
            .parse()
            .map_err(|error| ConfigError::new(format!("cannot quantize HMI signal: {error}")))?,
    );
    ...
    Ok(RuntimeSignal {
        ...
        tag: tag.into(),
        unit: unit.into(),
        minimum: super::quantities::quantize(minimum)?,
        maximum: super::quantities::quantize(maximum)?,
        value: *value,
