...
use super::super::quantities::quantize;
use super::super::state::{MouldProcessRuntime, RobotRuntime, RuntimeParameter};
...
pub(super) fn forming_moulds(
    ...
    let value = |tag| forming_initial_value(appliances, environment, zone, tag).and_then(quantize);
    let station_tags = [
        (
            "area-02-pt-02",
            "area-02-tt-02",
        ...
        station_tags.into_iter().enumerate()
    {
        ...
        let measurements = FormingMeasurements {
            ...
            piece_gripped: value("area-02-pe-01")? >= fixed!(0.5),
            piece_moisture_percent: fixed!(20.5),
            predicted_drying_shrinkage_percent: fixed!(2.1),
            drying_energy_factor: FixedValue::ONE,
...
fn forming_setpoints(
    ...
    Ok(FormingSetpoints {
        fill_ms: value("-fill-ms")?.round_to_u64(),
        pressure_bar: value("-pressure-bar")?,
        dwell_ms: value("-dwell-ms")?.round_to_u64(),
        drain_ms: value("-drain-ms")?.round_to_u64(),
        pickup_delay_ms: value("-pickup-delay-ms")?.round_to_u64(),
        wash_ms: value("-wash-ms")?.round_to_u64(),
        vacuum_ms: value("-vacuum-ms")?.round_to_u64(),
    })
}
