...
impl PlantControlRuntime {
    pub(super) fn set_control_mode(
        ...
        let Some(station) = self.controller.stations.get_mut(&self.context.id) else {
            return self.mode_result(
                HmiActionStatus::Denied,
                mode,
    ...
    pub(super) fn set_parameter(&mut self, parameter_id: String, value: f64) -> HmiActionReport {
        ...
        let Ok(value) = quantize(value) else {
            return self.parameter_result(
                HmiActionStatus::Denied,
                &parameter_id,
                "Parameter value is not a finite, representable engineering value.",
                "denied",
            );
        };
