    ...
    fn authorized_guarded_motion_target(&self, action: &HmiAction) -> Option<String> {
        match action {
            ...
            HmiAction::JogRobot {
                ...
                increment,
                speed_percent,
            } if valid_jog_increment(*coordinate_system, *axis, *increment)
                && valid_speed(*speed_percent)
...
fn valid_speed(speed_percent: f64) -> bool {
    robot_percentage(speed_percent).is_ok_and(|value| value.raw() > 0 && value.raw() <= 10_000)
}

fn valid_pose(pose: HmiRobotPose) -> bool {
    robot_pose(pose).is_ok()
}

fn valid_jog_increment(
    coordinate_system: HmiRobotCoordinateSystem,
    axis: HmiRobotAxis,
    increment: f64,
) -> bool {
    match (coordinate_system, axis) {
        (HmiRobotCoordinateSystem::World, HmiRobotAxis::X | HmiRobotAxis::Y | HmiRobotAxis::Z) => {
            robot_position(increment).is_ok_and(|value| value.raw() != 0)
        }
        (HmiRobotCoordinateSystem::World, HmiRobotAxis::W | HmiRobotAxis::P | HmiRobotAxis::R)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J1)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J2)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J3)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J4)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J5)
        | (HmiRobotCoordinateSystem::Joint, HmiRobotAxis::J6) => {
            robot_angle(increment).is_ok_and(|value| value.raw() != 0)
        }
        _ => false,
    }
}
