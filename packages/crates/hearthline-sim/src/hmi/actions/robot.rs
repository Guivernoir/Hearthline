use hearthline_engine::{RobotCartesianAxis, RobotCartesianIncrement, RobotMotionError};
use hearthline_model::{ComponentId, Percentage};

use super::super::builder::support::trace_entry;
use super::super::state::{
    PlantControlRuntime, robot_angle, robot_percentage, robot_pose, robot_position,
};
use super::super::{
...
impl PlantControlRuntime {
    ...
    pub(super) fn move_robot(
        ...
        let Ok(target) = robot_pose(target) else {
            return self.robot_result(
                HmiActionStatus::Denied,
                "cartesian-target",
                "Cartesian target coordinates must all be finite values.",
                "denied",
            );
        };
        let result = robot_speed(speed_percent).and_then(|speed_percent| {
            self.robot
                .as_mut()
                .expect("robot authority requires runtime")
                .command_pose(target, speed_percent)
        });
    ...
    pub(super) fn jog_robot(
        ...
        let robot = self
            ...
            .as_mut()
            .expect("robot authority requires runtime");
        let speed = robot_speed(speed_percent);
        let result = match coordinate_system {
            HmiRobotCoordinateSystem::World => cartesian_axis(axis)
                ...
                .and_then(|axis| {
                    let increment = match axis {
                        RobotCartesianAxis::X | RobotCartesianAxis::Y | RobotCartesianAxis::Z => {
                            let value = robot_position(increment)?;
                            if value.raw() == 0 {
                                return Err(RobotMotionError::InvalidIncrement);
                            }
                            RobotCartesianIncrement::Linear(value)
                        }
                        RobotCartesianAxis::W | RobotCartesianAxis::P | RobotCartesianAxis::R => {
                            let value = robot_angle(increment)?;
                            if value.raw() == 0 {
                                return Err(RobotMotionError::InvalidIncrement);
                            }
                            RobotCartesianIncrement::Angular(value)
                        }
                    };
                    speed.and_then(|speed| robot.jog_cartesian(axis, increment, speed))
                }),
            HmiRobotCoordinateSystem::Joint => joint_axis(axis)
                .ok_or(RobotMotionError::OutsideWorkspace)
                .and_then(|axis| {
                    let increment = robot_angle(increment)?;
                    if increment.raw() == 0 {
                        return Err(RobotMotionError::InvalidIncrement);
                    }
                    speed.and_then(|speed| robot.jog_joint(axis, increment, speed))
                }),
        };
    ...
    fn motion_result(
        ...
        let (status, message, outcome) = match result {
            ...
            Err(RobotMotionError::InvalidIncrement) => (
                HmiActionStatus::Denied,
                "Jog increment does not match the selected Cartesian axis.",
                "denied",
            ),
...
fn robot_speed(value: f64) -> Result<Percentage, RobotMotionError> {
    let value = robot_percentage(value)?;
    if value > Percentage::ZERO && value <= Percentage::from_raw(10_000) {
        Ok(value)
    } else {
        Err(RobotMotionError::InvalidSpeed)
    }
