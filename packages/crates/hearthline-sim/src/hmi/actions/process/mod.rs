...
use super::super::state::PlantControlRuntime;
...
pub(crate) fn validate_structured_text_syntax(source: &str) -> Result<(), crate::ConfigError> {
...
impl PlantControlRuntime {
    pub(super) fn start_process(&mut self) -> HmiActionReport {
        if self.body_preparation.is_some() {
            ...
            if self.controller.id == "area-01-vplc-01"
                && let Some(program) = self.controller.program.as_mut()
            {
                program.execute_scan(hearthline_engine::SequenceInputs {
                    start_request: true,
                    safety_ready,
                    ..hearthline_engine::SequenceInputs::default()
                });
                self.body_preparation
                    .as_mut()
                    .expect("Body Preparation runtime exists")
                    .apply_control_state(program.body_preparation_control_state());
            }
            self.tick(0);
            return self.body_action_result(
