use hearthline_engine::{
    BodyPreparationControlState, BodyPreparationPhysicsFeedback, FormingControlState, FormingPhase,
    FormingPhysicsFeedback, SequenceInputs, SequenceRuntime, SequenceScan, SlipPhase,
};
...
use super::binding::{
    ControlBindingConfig, ControlInputSource, body_preparation_phase, forming_phase,
};
...
impl ConfiguredControlProgram {
    ...
    pub(crate) fn phase(&self) -> FormingPhase {
        forming_phase(self.phase_name()).expect("compiled Forming phase name is valid")
    }

    pub(crate) fn body_preparation_phase(&self) -> SlipPhase {
        body_preparation_phase(self.phase_name())
            .expect("compiled Body Preparation phase name is valid")
    }
    ...
    pub(crate) fn control_state(&self) -> FormingControlState {
        FormingControlState {
            phase: self.phase(),
            running: self.runtime.running(),
            scan_count: self.runtime.scan_count(),
            cycle_count: self.runtime.cycle_count(),
        }
    }

    pub(crate) fn body_preparation_control_state(&self) -> BodyPreparationControlState {
        BodyPreparationControlState {
            phase: self.body_preparation_phase(),
            running: self.runtime.running(),
            scan_count: self.runtime.scan_count(),
            batch_count: self.runtime.cycle_count(),
        }
    }

    pub(crate) fn scan_with_physics_feedback(
        &mut self,
        elapsed_ms: u64,
        inputs: SequenceInputs,
        feedback: FormingPhysicsFeedback,
    ) -> Option<SequenceScan> {
        let timer_override_ms = if feedback.phase_complete {
            Some(0)
        } else {
            Some(u64::MAX)
        };
    ...
    pub(crate) fn scan_with_body_preparation_feedback(
        &mut self,
        elapsed_ms: u64,
        inputs: SequenceInputs,
        feedback: BodyPreparationPhysicsFeedback,
    ) -> Option<SequenceScan> {
        let timer_override_ms = if feedback.phase_complete {
            Some(0)
        } else {
