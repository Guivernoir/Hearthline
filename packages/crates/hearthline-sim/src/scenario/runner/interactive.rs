...
use crate::{
    ...
    ScenarioRuntimeSnapshots, ScenarioStateSnapshot,
};
...
impl InteractiveScenarioSession {
    ...
    pub fn run(
        ...
        let mut initial_runtime = ScenarioStateSnapshot::capture(&self.network, self.now_us);
        initial_runtime.captured_at_us = 0;
        ...
        let final_time_us = trace.last().map_or(started_at_us, |entry| entry.time_us);
        let mut final_runtime = ScenarioStateSnapshot::capture(&self.network, final_time_us);
        final_runtime.captured_at_us = final_time_us.saturating_sub(started_at_us);
        self.now_us = final_time_us.saturating_add(1);
...
pub fn is_interactive_scenario(scenario: &ScenarioConfig, source: &str) -> bool {
