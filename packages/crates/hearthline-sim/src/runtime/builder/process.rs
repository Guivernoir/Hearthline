...
use hearthline_config::{ApplianceConfig, BehaviorConfig, ConfigError, ConfigRepository};
...
pub(super) fn build_process_appliance(
    ...
    match &config.behavior {
        ...
        BehaviorConfig::FieldSensor {
            ...
            sensor.set_raw_value(quantize(initial_value.unwrap_or(0.0))?);
...
fn quantize(value: f64) -> Result<FixedValue, ConfigError> {
    format!("{value:.6}")
        .parse()
        .map_err(|error| ConfigError::new(format!("cannot quantize analog value: {error}")))
}
