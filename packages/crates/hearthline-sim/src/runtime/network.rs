...
use hearthline_config::ConnectionRepository;
use hearthline_config::{ConfigError, ConfigRepository, InterfaceMode};

use super::media::build_media_link;
use super::{ConfiguredAppliance, RuntimeDeviceSnapshot, build_appliance};
use crate::{CELL_SNAPSHOT_SCHEMA_VERSION, ComponentSnapshot, SnapshotValue};
...
impl ConfiguredNetwork {
        ...
        S: AsRef<str>,
    {
        ...
        let links = selected_connections
            .into_iter()
            .map(|connection| build_media_link(&connection.config, appliances))
    ...
    pub fn runtime_snapshot(&self, now_us: u64) -> Vec<RuntimeDeviceSnapshot> {
        self.appliances
            .iter()
            .map(|appliance| appliance.runtime_snapshot(now_us))
            .collect()
    }

    pub fn runtime_link_snapshot(&self) -> Vec<RuntimeLinkSnapshot> {
        self.links
            .iter()
            .map(|link| RuntimeLinkSnapshot {
                id: link.id().to_string(),
                authoritative_state: format!("{link:?}"),
            })
            .collect()
    }

    pub(crate) fn component_snapshots(&self, now_us: u64) -> Vec<ComponentSnapshot> {
        let mut components = self
            .runtime_snapshot(now_us)
            .into_iter()
            .map(|device| {
                let runtime_state = serde_json::to_string(&device)
                    .expect("runtime device snapshots are serializable");
                ComponentSnapshot {
                    schema_version: CELL_SNAPSHOT_SCHEMA_VERSION.into(),
                    component: device.id,
                    component_kind: device.kind,
                    state: BTreeMap::from([
                        (
                            "authoritative-state".into(),
                            SnapshotValue::Text(device.authoritative_state),
                        ),
                        (
                            "firewall-session-count".into(),
                            SnapshotValue::Integer(device.firewall_sessions.len() as i64),
                        ),
                        (
                            "mac-entry-count".into(),
                            SnapshotValue::Integer(device.mac_table.len() as i64),
                        ),
                        (
                            "neighbor-count".into(),
                            SnapshotValue::Integer(device.neighbors.len() as i64),
                        ),
                        (
                            "pat-entry-count".into(),
                            SnapshotValue::Integer(device.pat_translations.len() as i64),
                        ),
                        ("runtime-state".into(), SnapshotValue::Text(runtime_state)),
                        (
                            "supports-firewall-sessions".into(),
                            SnapshotValue::Boolean(device.supports_firewall_sessions),
                        ),
                        (
                            "supports-mac-table".into(),
                            SnapshotValue::Boolean(device.supports_mac_table),
                        ),
                        (
                            "supports-neighbors".into(),
                            SnapshotValue::Boolean(device.supports_neighbors),
                        ),
                        (
                            "supports-pat".into(),
                            SnapshotValue::Boolean(device.supports_pat),
                        ),
                    ]),
                }
            })
            .collect::<Vec<_>>();
        components.extend(
            self.runtime_link_snapshot()
                .into_iter()
                .map(|link| ComponentSnapshot {
                    schema_version: CELL_SNAPSHOT_SCHEMA_VERSION.into(),
                    component: link.id,
                    component_kind: "connection-media".into(),
                    state: BTreeMap::from([(
                        "authoritative-state".into(),
                        SnapshotValue::Text(link.authoritative_state),
                    )]),
                }),
        );
...
pub struct RuntimeLinkSnapshot {
    pub id: String,
