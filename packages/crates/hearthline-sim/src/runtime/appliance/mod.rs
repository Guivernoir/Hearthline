...
use hearthline_engine::{
    ...
    LinkAppliance, NatRouter, NeighborEntry, OperatorInterface, PassiveSensor, RemoteIo,
    ReverseProxyWaf, Router, SafetyInterface, ServiceNode, SimulatedComponent, SimulationEvent,
    StatefulFirewall, UnaddressedNode, VirtualPlc, WirelessAccessPoint,
};
...
pub use snapshot::{
    RuntimeDeviceSnapshot, RuntimeFirewallSessionEntry, RuntimeMacEntry, RuntimeNeighborEntry,
    RuntimePatEntry,
};
...
pub enum ConfiguredAppliance {
    ...
    Wireless(Box<WirelessAccessPoint>),
    Monitor(Box<PassiveSensor>),
    Unaddressed(Box<UnaddressedNode>),
...
const _: () = assert!(
    core::mem::size_of::<ConfiguredAppliance>()
        <= hearthline_engine::RUNTIME_COMPONENT_HANDLE_BYTES
);
...
impl SimulatedComponent for ConfiguredAppliance {
    fn id(&self) -> &ComponentId {
        match self {
            ...
            Self::Wireless(appliance) => appliance.id(),
            Self::Monitor(appliance) => appliance.id(),
            Self::Unaddressed(appliance) => appliance.id(),
    ...
    fn kind(&self) -> ComponentKind {
        match self {
            ...
            Self::Wireless(appliance) => appliance.kind(),
            Self::Monitor(appliance) => appliance.kind(),
            Self::Unaddressed(appliance) => appliance.kind(),
    ...
    fn has_port(&self, port: &PortId) -> bool {
        match self {
            ...
            Self::Wireless(appliance) => appliance.has_port(port),
            Self::Monitor(appliance) => appliance.has_port(port),
            Self::Unaddressed(appliance) => appliance.has_port(port),
    ...
    fn handle(&mut self, event: SimulationEvent) -> EffectList {
        match self {
            ...
            Self::Wireless(appliance) => appliance.handle(event),
            Self::Monitor(appliance) => appliance.handle(event),
            Self::Unaddressed(appliance) => appliance.handle(event),
