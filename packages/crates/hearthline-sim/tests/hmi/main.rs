...
use hearthline_config::{
    ...
    HmiPreparationTrain, HmiProcessFault, HmiRobotAxis, HmiRobotCoordinateSystem, HmiRobotPose,
    ScenarioApplicationConfig, ScenarioConnectionOverride, ScenarioPacketConfig,
    ScenarioRepository, retarget_telemetry_packet, telemetry_identity,
};
use hearthline_engine::{HistorianBuffer, PUMP_HEARTBEAT_TIMEOUT_MS};
use hearthline_sim::{
    PlantControlRuntime, PlantRuntimeStore, build_forming_telemetry_packet,
...
#[test]
fn hmi_construction_and_projection_fit_a_constrained_worker_stack() {
    std::thread::Builder::new()
        .name("hmi-stack-budget".into())
        .stack_size(512 * 1024)
        .spawn(|| {
            assert!(core::mem::size_of::<PlantControlRuntime>() <= 2 * 1024);
            let appliances = repository();
            let mut sessions = PlantRuntimeStore::default();
            let snapshot = sessions
                .profile(&appliances, "area-01-wt-hmi-01")
                .expect("Body Preparation HMI profile");
            assert_eq!(snapshot.environment, "Body Preparation");
        })
        .expect("spawn constrained HMI worker")
        .join()
        .expect("constrained HMI worker completed");
}

#[test]
fn plant_store_owns_one_runtime_for_many_operator_contexts() {
    let appliances = repository();
    let mut sessions = PlantRuntimeStore::default();
    sessions
        .profile(&appliances, "area-01-hmi-01")
        .expect("Body Preparation cell");
    let body_preparation = sessions.ownership();
    assert_eq!(body_preparation.plant_runtimes, 1);
    assert!(body_preparation.operator_contexts > 1);
    assert!(body_preparation.controller_runtimes > 1);

    sessions
        .profile(&appliances, "area-02-machine-pc-01")
        .expect("Forming cell");
    let with_forming = sessions.ownership();
    assert_eq!(with_forming.plant_runtimes, 2);
    assert!(with_forming.operator_contexts > body_preparation.operator_contexts);
    assert_eq!(
        with_forming.controller_runtimes,
        body_preparation.controller_runtimes + 1
    );
}
...
fn body_preparation_hmi_is_built_from_area_configuration() {
    let appliances = repository();
    let session =
        PlantControlRuntime::from_repository(&appliances, "area-01-hmi-01").expect("HMI session");
...
fn every_process_hmi_executes_each_configured_field_state() {
    ...
    for hmi_id in HMI_IDS {
        let mut session =
            PlantControlRuntime::from_repository(&appliances, hmi_id).expect("interactive HMI");
