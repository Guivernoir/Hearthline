...
fn open_gate_denies_manual_mould_and_robot_motion() {
    let appliances = repository();
    let mut mould_sessions = PlantRuntimeStore::default();
    mould_sessions
        .execute(
            &appliances,
            "area-02-machine-pc-01",
    ...
    let mut robot_sessions = PlantRuntimeStore::default();
    robot_sessions
        .execute(
            &appliances,
            "area-02-machine-pc-01",
...
#[test]
fn open_gate_interlock_classifies_all_authorized_motion_families() {
    let appliances = repository();
    ...
    for (interface, action) in [
        ...
        let report = sessions
            ...
            .expect("invalid motion report");
        assert!(matches!(report.status, HmiActionStatus::Denied));
        assert!(
            !guard(&report.snapshot).reset_required,
            "invalid or unauthorized commands must not create a guard trip"
        );
    }

    for action in [HmiAction::StartMould, HmiAction::EndMouldAfterCycle] {
        let mut sessions = PlantRuntimeStore::default();
        sessions
            .execute(
                &appliances,
        ...
        let report = sessions
            ...
            .expect("guarded mould action report");
        assert!(matches!(report.status, HmiActionStatus::Denied));
        assert!(guard(&report.snapshot).reset_required);
    }
    ...
    for action in [
        ...
        sessions
            ...
            .expect("open access gate");
        let report = sessions
            .execute(&appliances, "area-02-joystick-01", action)
            .expect("guarded robot action report");
        assert!(matches!(report.status, HmiActionStatus::Denied));
        assert!(guard(&report.snapshot).reset_required);
    }
    ...
    for action in [HmiAction::RunRobotProgram, HmiAction::StepRobotProgram] {
        ...
        let report = sessions
            .execute(&appliances, "area-02-joystick-01", action)
            .expect("guarded robot-program report");
        assert!(matches!(report.status, HmiActionStatus::Denied));
        assert!(guard(&report.snapshot).reset_required);
    }
}
...
fn configured_handoffs_begin_inside_the_guarded_cell() {
    let appliances = repository();
    let snapshot = PlantControlRuntime::from_repository(&appliances, "area-02-machine-pc-01")
